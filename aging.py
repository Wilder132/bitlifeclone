...
def age_up(character):
    ...
    for disease, health_impact, _, chance in char_base.diseases:
        if random.randint(1, 100) <= chance and disease not in character.diseases:
            character.diseases.append(disease)
            print(f"\n--- Oh no! ---\nYou have contracted {disease}.")
            input("Press Enter to continue...")

    # --- Apply effects of current diseases ---
    for disease, health_impact, _, _ in char_base.diseases:
        if disease in character.diseases:
            character.c_health = max(1, character.c_health + health_impact)
